...
- [ ] 一张图从上传走到「已提取」，对 `ExtractionEngine.extract` 的调用次数为 1
- [ ] 原图取回与 PDF 选定页栅格化在一次处理里也只做一次
- [ ] 分级结果仍先落库并 commit，**报价员**在取数期间能看到已分级状态
- [ ] 分级本身失败时重试仍从分级开始，不跳过劝退分支
- [ ] 差图被显式覆盖继续时不产生额外的引擎调用（或在票里写明为何选择重新调用）
- [ ] 缝 1：用计数型假引擎断言单张图的调用次数，这个不变量被测试锁住而不是靠人读代码
- [ ] 缝 1 既有测试全部不改仍然通过
