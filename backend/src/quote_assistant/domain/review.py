...
def field_risk_class(field_key: str) -> FieldRiskClass:
    return FIELD_RISK_CLASS_BY_KEY.get(field_key, FieldRiskClass.DIMENSION)
...
def review_fields_for(drawing: PartDrawing) -> tuple[ReviewFieldView, ...]:
    return tuple(
        ...
        for field in merge_extracted_fields(drawing.extracted_fields)
    )
...
def _require_canonical_field(field_key: str) -> None:
    if field_key not in CANONICAL_FIELD_BY_KEY:
...
def _replace_field(
    ...
    *,
    ...
    _require_canonical_field(field_key)
